...
def compute_call_graph(binary: Binary, program: Program) -> None:
    ...
    and fill the call attribute of binary.
    ...
    for f_addr, f in program.items():
        ...
        if exports.get(f_addr) or exports.get(f_addr+1):  # If the function is somewhat exported (and visible in LIEF)
            res = exports.get(f_addr, exports.get(f_addr+1))  # In THUMB mode address is address+1
...
def get_lief_exports_mapping(binary: Binary) -> tuple[
