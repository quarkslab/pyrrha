        ...
        with Progress(
            ...
            for i, binary in enumerate(self.fs.iter_binaries()):
                ...
                for f_name, targets in binary.calls.items():
                    f_symbol: Symbol = binary.functions[f_name]
...
are not defined"
    ...
    def _record_one_call(
        ...
        # call not among libraries (need to find it)
            ...
            if len(served_by) > 1:
                if choice := self.try_resolve_duplicate_symbols(
                    binary, callee, served_by, resolver, resolve_cache
                ):
    ...
    def try_resolve_duplicate_symbols(
        self,
        binary: Binary,
        callee: str,
        candidates: list[Binary],
        resolver: ResolveDuplicateOption,
        resolve_cache: set[Binary],
    ) -> Binary | None:
        """Try to discriminate the right destination of a call.

        This happens when two libraries are exposing the same symbol.
            ...
            with hide(self.progress):
                ...
                for cache_entry in resolve_cache:
                    if cache_entry in served_by:  # reuse already selected entry
                        logging.debug(
                            f"reuse manually selected entry to disambiguate {callee}"
                        )
                        served_by = [cache_entry]
                if len(served_by) > 1:  # still not resolved
                    print(
                        f"symbol {callee} needed for {binary.path} served by multiple binaries:"
                    )
                    val = None
