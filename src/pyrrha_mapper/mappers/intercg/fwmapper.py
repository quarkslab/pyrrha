...
def map_firmware(db: SourcetrailDB, root_path: Path, dump: PyrrhaDump, jobs: int, resolver: ResolveDuplicateOption) -> bool:
    ...
    for i, binary in enumerate(all_bins):
        logging.info(f"[{i + 1}/{len(all_bins)}] index node: {binary.name}")
        ...
        cmd = ["numbat-ui", str(abs_path)+".srctrlprj"]
    ...
    for i, (pyr_id, binary) in enumerate(binary_mapping.items()):
        ...
        logging.info(f"-------------- [{i+1}/{tot}] index call graph {binary.path} [pyr_id:{pyr_id}] --------------")
                    ...
                    if target in binary.calls:  # local call
                        ...
                        if target not in IGNORE_LIST:
                            ...
                            if len(served_by) == 1:  # Automatically add the lib to deps_symbols
                                logging.info(f"symbol {target} served by {served_by[0].name} automatically add it!")
